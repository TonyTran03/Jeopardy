...
export default function Create() {
  ...
  useEffect(() => {
    if (initialized) {
      ...
      navigate(
        `?grid=${serializedGrid}&rows=${rows}&cols=${cols}&columnNames=${serializedColumnNames}`,
        { replace: true }
      );
    }
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="flex h-screen w-screen">
        <div className="sidebar flex flex-col flex-1 p-4 bg-gray-200">
          ...
          {isGridFilled && (
            <button
              className="save-button bg-blue-500 text-white p-4 rounded mt-4"
